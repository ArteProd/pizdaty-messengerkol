        ...
        <script>
                ...
                ws.onmessage = (event) => {
                    try {
                        ...
                        if (data.type === 'new_message') {
                            ...
                            loadChats(); // Просто обновляем список
                        }
            ...
            function updateChatInList(chatId, lastMessage) {
                ...
                if (lastMsgEl && lastMessage) {
                    ...
                    lastMsgEl.style.transition = 'opacity 0.2s ease';
                    ...
                    setTimeout(() => {
                        ...
                        lastMsgEl.style.opacity = '1';
                    }, 200);
                }
                
                // Обновляем галочки для нового сообщения
                const timeContainer = chatItem.querySelector('.chat-item-time');
                if (timeContainer && lastMessage) {
                    // Новое сообщение всегда отправлено (is_read = false при отправке)
                    // Показываем 1 галочку - сообщение отправлено, но не прочитано
                    const checkmarksHtml = '<span class="chat-item-checkmarks">✓</span>';
                    // Форматируем время
            ...
            function addMessageToChat(message) {
                ...
                // Проверяем дату последнего сообщения в контейнере
                let lastMsgDate = null;
                const messageElements = container.querySelectorAll('.message');
                if (messageElements.length > 0) {
                    // Находим последнее реальное сообщение в DOM
                    const lastElement = messageElements[messageElements.length - 1];
                    const lastMsgUuid = lastElement.getAttribute('data-id');
                    const lastMsg = messages.find(m => m.uuid === lastMsgUuid);
                    if (lastMsg) {
                        lastMsgDate = new Date(new Date(lastMsg.timestamp).getTime() + 3*60*60*1000).toDateString();
                    }
                }
                
                const msgDate = new Date(new Date(message.timestamp).getTime() + 3*60*60*1000).toDateString();
                
                // Добавляем разделитель даты если:
                // 1. Это первое сообщение в массиве
                // 2. ИЛИ дата изменилась по сравнению с последним сообщением
                const isFirstMessage = messages.length === 0;
                const needsDateSeparator = (isFirstMessage && !existingDateSeparator) || msgDate !== lastMsgDate;
    ...
    document.getElementById('userAvatar').textContent = updatedUser.username[0].toUpperCase();
            ...
            async function deleteMessage(uuid) {
                ...
                setTimeout(() => {
                    ...
                    // Перерисовываем даты если нужно
                    renderMessages();
                }, 300);
